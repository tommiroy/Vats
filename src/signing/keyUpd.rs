...
// ) {
    
    {
        ...
        let back_tostring_commitments = serde_json::from_str::<Vec<String>>(&commitments_string).unwrap();
        ...
        //verify
        assert_eq!(sigma_i.0, back_to_point_sigma_i, "sigma_i.0: {:?}, back_to_point_sigma_i: {:?}", sigma_i.0, back_to_point_sigma_i);
        assert_eq!(sigma_i.1, back_to_scalar_sigma_i, "sigma_i.1: {:?}, back_to_scalar_sigma_i: {:?}", sigma_i.1, back_to_scalar_sigma_i);
...
fn verify_sigma(sender: Signer, sigma: (RistrettoPoint, Scalar), context_string: String) -> bool{
    ...
    assert_eq!(big_r, &RISTRETTO_BASEPOINT_TABLE*&mu + sender.public_key.key*c.invert(), 
