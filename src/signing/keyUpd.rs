...
pub fn verify_new_share(me: &mut Client, sender_id: u32, new_share: Scalar) {
    ...
    if me.commitments.contains_key(&sender_id) {
        ...
        for (k, &big_a) in me.commitments.get(&sender_id).expect("keyUpd-verify_new_share: cannot get id").iter().enumerate() {
            ...
            lhs += big_a*Scalar::from(sender_id)*Scalar::from(k as u32);
        }
    }

    if &RISTRETTO_BASEPOINT_TABLE * &new_share == lhs {
        ...
        // big_a*Scalar::from(sender_id)*Scalar::from(k as u32);
...
pub async fn update_share(signer:&mut Client, participants: Vec<u32>, t: usize, context: String) -> bool{
    ...
    for participant in participants {
        if new_shares.contains_key(&participant) {
            let msg = Message {sender: signer.id.to_string(), receiver: participant.to_string(), msg_type: MsgType::KeyUpdNewShare, msg: vec![scalar_to_string(new_shares.get(&participant).expect("keyUpd: Cannot find participant"))]};
